...
if (!$course_id || !$conn->query("SELECT id FROM user_courses WHERE student_id = $student_id AND course_id = $course_id")->num_rows) {
    header("Location: dashboard.php");
    exit();
}

$course = $conn->query("SELECT c.title, c.content_type, c.content_path, c.created_at, c.difficulty, s.name AS subject, l.name AS level 
                        FROM courses c 
                        JOIN subjects s ON c.subject_id = s.id 
                        JOIN levels l ON c.level_id = l.id 
                        WHERE c.id = $course_id")->fetch_assoc();
$embed_url = $course['content_type'] === 'video' ? $course['content_path'] : '';
?>
